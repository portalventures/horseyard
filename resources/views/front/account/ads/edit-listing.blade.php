...
@section('title', 'Horseyard | Edit Ad')
  ...
  <div id="main">
    <section id="inbox" class="onlyRight">
      <div class="container">
        ...
        <div class="inbox-right ads-edit">
          <form class="user_update_ad_form">
            @csrf
            <input type="hidden" class="ad_slug_url" id="ad_slug_url" name="ad_slug_url" value="{{$ad_data->slug}}">
                            ...
                            <div class="d-flex flex-column">
                              @foreach($all_top_categories as $key => $top_categories)
                                <label for="cat-{{$top_categories->category_name}}" class="custom-radio all_top_categories">
                                  <input type="radio" name="listing_category" value="{{$top_categories->id}}" 
                                  {{ $ad_data->category_id == $top_categories->id ? 'checked' : '' }} id="cat-{{$top_categories->category_name}}" data-id="{{strtolower($top_categories->category_name)}}">
                                  <span></span>
                      ...
                      <div class="form-group">
                        <div class="row">
                          ...
                          <div class="col col-md-4 mb-3 mb-md-0">
                            <div class="d-flex flex-column">
                              <input type="text" class="form-control check-validity item_price" placeholder="Enter Price" required name="price" id="price" value="{{$ad_data->price}}" onkeypress='return event.charCode >= 48 && event.charCode <= 57'>
                            ...
                            <div class="d-flex mt-2 flex-column flex-md-row">
                              ...
                              <div class="d-flex flex-column flex-md-row">
                                <label for="item-free" class="custom-radio mr-3 mb-0">
                                  <input type="radio" name="item_show_type" id="item-free" class="item_type" value="free" {{ $ad_data->item_show_type == 'free' ? 'checked' : '' }} >
                                  <span></span>
                                  Free
                                </label>
                                <label for="item-negotiable" class="custom-radio mr-3 mb-0">
                                  <input type="radio" name="item_show_type" id="item-negotiable" class="item_type" value="negotiable" {{ $ad_data->item_show_type == 'negotiable' ? 'checked' : '' }}>
                                  <span></span>
                                  Negotiable
                                </label>
                                <label for="item-ono" class="custom-radio mr-3 mb-0">
                                  <input type="radio" name="item_show_type" id="item-ono" class="item_type" value="ONO" {{ $ad_data->item_show_type == 'ONO' ? 'checked' : '' }}>
                                  <span></span>
                                  ONO
                                </label>
                              </div>
                    ...
                    <div class="step-content">
                            ...
                            <div class="selectParent">
                              <select name="state" id="state" class="form-control check-validity state_list" required>
                                ...
                                @foreach($all_state as $key => $value)
                                  <option value="{{$value->id}}" {{ $ad_data->state_id == $value->id ? 'selected' : '' }}>{{$value->state_name}}</option>
                                @endforeach
                              </select>
                              <span class="errorMessage"></span>
                            </div>
                            ...
                            <div class="selectParent suburb_div">
                              <select name="suburb" id="suburb" class="form-control check-validity" required>
                                @foreach($suburb_list as $key => $suburb)
                                  <option value="{{$suburb->id}}" {{ $ad_data->suburb_id == $suburb->id ? 'selected' : '' }}>{{$suburb->suburb_name}}</option>
                                @endforeach   
                              </select>
                              <span class="errorMessage"></span>
                            </div>
                      ...
                      <div class="form-group">
                        <div class="row">
                          ...
                          <div class="col col-md-4 mb-3 mb-md-0">
                            <div class="d-flex flex-column">
                              <input type="text" placeholder="PIC Number" class="form-control" name="pic_number" id="pic_number" value="{{ $ad_data->pic_number }}">
                      ...
                      <div class="form-group">
                        <div class="row">
                          ...
                          <div class="col col-md-10 mb-3 mb-md-0">
                             <input type="text" placeholder="Title of Ad" class="form-control check-validity firstNoSpace" name="title" id="title" required value="{{ $ad_data->title }}">
                            <span class="errorMessage"></span>
                          </div>
                        </div>
                      </div>
                      <div class="form-group">
                        <div class="row">
                          ...
                          <div class="col col-md-10">
                            <textarea name="description" id="description" cols="30" rows="5" placeholder="Description of Ad" class="form-control check-validity firstNoSpace" required>{{ $ad_data->description }}</textarea>
                            <span class="errorMessage"></span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="dropzone">
                        <div id="list">
                          @foreach($ad_images as $key => $image)
                            <span>
                              <a class="icon red-trash delete_ad_image" data-id="{{$image->id}}"></a>
                              <img src="{{ url($image->image_url.'/'.$image->image_name) }}" alt="Featured listing" class="thumb img-fluid" />
                            </span>
                          @endforeach
                          <span class="add-more-image">Add images</span>
                        </div>
                          ...
                          <div class="col col-md-4 mb-3 mb-md-0">
                            <div class="d-flex flex-column flex-md-row">
                              <div class="field-with-icon right w-100">
                                <input type="text" placeholder="Enter URL" class="form-control" name="video_url" id="video_url" value="{{$ad_data->video_url}}">
                    ...
                    <div class="step-content">
                      <div class="form-group">
                        <div class="row">
                          ...
                          <div class="col col-md-4 mb-3 mb-md-0">
                            <div class="d-flex flex-column">
                              <input type="text" placeholder="Name" name="contact_name" id="contact_name" class="form-control check-validity firstNoSpace" required  value="{{$ad_data->contact_name}}">
                              <span class="errorMessage"></span>
                            </div>
                          </div>
                          <div class="col col-md-2 pr-md-0">
                          ...
                          <div class="col col-md-4 mb-3 mb-md-0">
                            <div class="d-flex flex-column">
                              <input type="text" placeholder="Enter Mobile Number" name="contact_number" id="contact_number" class="form-control check-validity firstNoSpace" required  value="{{$ad_data->contact_number}}">
                              <span class="errorMessage"></span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="form-group">
                        <div class="row">
                          ...
                          <div class="col col-md-4 mb-3 mb-md-0">
                            <div class="d-flex flex-column">
                              <input type="email" placeholder="Enter Email Address" name="contact_email" id="contact_email" class="form-control check-validity firstNoSpace" required  value="{{$ad_data->contact_email}}">
                            </div>
                          </div>
                        </div>
                      </div>
