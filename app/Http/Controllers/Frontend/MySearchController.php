...
class MySearchController extends Controller
{
  ...
  {    
    ...
    if(isset($mParams['discipline']) && !empty($mParams['discipline'])){
      ...
      if(count($dIds) > 0) {
        $searchParams->selectedDiscipline = $dIds;
        $lIds = getListingDynamicFieldUsingIds($dIds,$count_filters);
        $search_result = $search_result->whereIn('listing_master.id', $lIds);      
      }
    }
    ...
    if(isset($mParams['breed']) && !empty($mParams['breed'])){
      ...
      //$dIds = getDynamicFieldUsingSlugArr($pParams);
      $dIds = getDynamicFieldUsingSlugArr_Breed_Primary($pParams);      
      // if(count($dIds) > 0) {
    ...
    if(isset($mParams['rider_level']) && !empty($mParams['rider_level'])){
      ...
      $dIds = getDynamicFieldUsingSlugArr($pParams);
      // if(count($dIds) > 0) {
      //   $searchParams->selectedRiderLevel = $dIds;
      //   array_push($array_dynamic_ids, $dIds);
      //   $count_filters++;
      // }
      if(count($dIds) > 0) {
    ...
    if(((isset($mParams['field-height'])) || (isset($mParams['max-field-height']))) && $searchParams->top_category == 'horses')
    {
      $min_height = isset($mParams['field-height']) ? explode(DELIM_URL, $mParams['field-height']) : '';
      $max_height = isset($mParams['max-field-height']) ? explode(DELIM_URL, $mParams['max-field-height']) : '';
      
      $dIds = getDynamicField_HeightIdsArr($min_height,$max_height);
      
      if(isset($mParams['field-height']))
      {
        $min_height_id = DynamicFieldValues::where('field_id', 9)
                                           ->where('field_value',$mParams['field-height'])
                                           ->first();
      ...
      if(isset($mParams['max-field-height']))
      {
        $max_height_id = DynamicFieldValues::where('field_id', 9)
                                           ->where('field_value',$mParams['max-field-height'])
                                           ->first();
    ...
    if(((isset($mParams['min-age'])) || (isset($mParams['max-age']))) && $searchParams->top_category == 'horses')
    {
